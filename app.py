...
# Cache GPT-2 model loading to avoid slow reloads
@st.cache_resource
def load_gpt2_model():
...
# GenAI wrapper replacement to call GPT-2 locally
def call_genai_chat(prompt: str, max_length: int = 150, num_return_sequences: int = 1) -> str:
    try:
        outputs = gpt2_generator(prompt, max_length=max_length, num_return_sequences=num_return_sequences)
        return outputs[0]['generated_text']
    except Exception as e:
        return f"⚠️ GPT-2 inference error: {e}"

# --------------------------
# Sidebar: controls, scenario presets, and LLM options
# --------------------------
...
# Simplified LLM model selection for GPT-2 local
st.sidebar.subheader("GenAI (GPT-2) settings")
llm_model = st.sidebar.selectbox("Model", ["gpt2", "gpt2-medium", "gpt2-large"], index=0)
llm_temp = st.sidebar.slider("Temperature", 0.0, 1.0, 0.0, step=0.05)  # GPT-2 does support temperature
llm_max_tokens = st.sidebar.number_input("Max tokens", value=150, min_value=50, max_value=2000, step=50)

st.sidebar.markdown("---")
st.session_state['reduce_motion'] = st.sidebar.checkbox("Reduce motion (accessibility)", value=st.session_state['reduce_motion'])
st.sidebar.markdown("---")

# Remove the Groq client initialization and warnings code from your app entirely.

# Replace the existing call_genai_chat function with this GPT-2 version:

def call_genai_chat(prompt: str, model: str = "gpt2", max_length: int = 150,
                   num_return_sequences: int = 1, temperature: float = 0.0) -> str:
    try:
        outputs = gpt2_generator(prompt, max_length=max_length,
...
# Update all occurrences where call_genai_chat is called inside your Streamlit buttons or AI sections:

# Example for forecast explanation button:

if st.button("Explain forecast (AI)"):
    prompt = (f"County: {county_sel}. Provide a concise explanation (<=150 words) "
              f"of the forecast trend based on recent historical counts and the 3-year monthly forecast sample: "
              f"{forecast_df.head(6).to_dict(orient='records')}. Mention drivers and recommended planner actions.")
    ai_text = call_genai_chat(prompt, model=llm_model,
                             max_length=llm_max_tokens, temperature=llm_temp)
    st.session_state['last_ai'] = ai_text
    st.info(ai_text)

# Make similar replacements wherever AI calls occur (e.g., Executive summary, Q&A assistant, scenario narration),

# ensuring you replace 'max_tokens' with 'max_length' and pass 'temperature' too.
# --------------------------
# Load data & model remain unchanged
# --------------------------
df = load_csv_data("preprocessed_ev_data.csv")
model = load_model("forecasting_ev_model.pkl")

# --------------------------
# Your existing UI and visualization code below ... 
# Including KPI cards, national trends, county hotspot, forecast viewer,
# charger mix visualization, scenario analysis, AI Assistant & executive summary
# Replace only call_genai_chat calls as shown in block 3.

# Example for AI Assistant Q&A button:
...
if st.button("Ask AI"):
    ...
    answer = call_genai_chat(prompt, model=llm_model, max_length=llm_max_tokens, temperature=llm_temp)
...
if st.button("Generate Executive Summary"):
    ...
    summary = call_genai_chat(prompt, model=llm_model, max_length=llm_max_tokens, temperature=llm_temp)
    st.session_state['last_ai'] = summary
    st.info(summary)

# Latest AI output expander remains unchanged
...
# Footer and all other plots/UI parts remain unchanged. Just ensure AI calls use local GPT-2 wrapper.
...
# --------------------------
# Compute EV demand and infrastructure metrics
# --------------------------
...
st.markdown(
    ...
    f"<div class='small-muted'>Total Energy (GWh)</div>"
    f"<div style='font-size:20px;font-weight:700;color:#1b4332'>{total_gwh:,.2f}</div>"
    f"</div>",
    unsafe_allow_html=True)

st.markdown(
    f"<div class='kpi-card' style='flex:1'>"
    f"<div class='small-muted'>Estimated Chargers</div>"
    f"<div style='font-size:20px;font-weight:700;color:#1b4332'>{total_chargers:,}</div>"
    f"</div>",
    unsafe_allow_html=True)

st.markdown(
    f"<div class='kpi-card' style='flex:1'>"
    f"<div class='small-muted'>Aggregate Grid Load (MW)</div>"
    f"<div style='font-size:20px;font-weight:700;color:#1b4332'>{total_grid_mw:,.2f}</div>"
    f"</div>",
    unsafe_allow_html=True)

st.markdown(
    f"<div class='kpi-card' style='flex:1'>"
    f"<div class='small-muted'>Top County by Demand</div>"
    f"<div style='font-size:20px;font-weight:700;color:#1b4332'>{top_county}</div>"
    f"</div>",
    unsafe_allow_html=True)
...
# --------------------------
# National Trends & Infrastructure Needs Visualization
# --------------------------
st.header("National Trends & Infrastructure Needs")
col1, col2 = st.columns([3, 1])
with col1:
    st.subheader("National EV Charging Energy Demand (GWh) — Over Time")
    fig1 = px.line(infra_trend, x='Date', y='Monthly_GWh_Demand', markers=True,
                   labels={'Monthly_GWh_Demand': 'Monthly Demand (GWh)'})
...
# --------------------------
# County Hotspots & Prioritization
# --------------------------
...
fig2 = px.bar(county_demand_sorted, x='County', y='Monthly_GWh_Demand',
              hover_data=['Chargers_Required', 'Grid_Load_MW'],
              labels={'Monthly_GWh_Demand': 'Total Monthly Demand (GWh)'})
st.plotly_chart(fig2, use_container_width=True)

st.dataframe(
    county_demand_sorted.reset_index(drop=True).rename(columns={
        'Monthly_GWh_Demand': 'Total_GWh',
        'Chargers_Required': 'Chargers',
        'Grid_Load_MW': 'GridLoad_MW'
    })
)

# --------------------------
# County Forecast Viewer (3-year monthly forecast)
# --------------------------
st.markdown("---")
...
if county_sel:
    ...
    if county_df.empty:
        ...
        if model is None:
            ...
            ax.set_xlabel("Date")
            ax.set_ylabel("Cumulative EV Count")
            ...
            if st.button("Explain forecast (AI)"):
                prompt = (
                    f"County: {county_sel}. Provide a concise explanation (<=150 words) of the forecast trend "
                ...
                ai_text = call_genai_chat(prompt, model=llm_model, max_length=llm_max_tokens, temperature=llm_temp)
                st.session_state['last_ai'] = ai_text
                st.info(ai_text)
# --------------------------
# Charger Type Mix & Grid Impact
# --------------------------
st.markdown("---")
...
# --------------------------
# Scenario Analysis & AI Narration
# --------------------------
st.markdown("---")
...
if st.button("Generate AI Narrative (Scenarios)"):
    ...
    llm_resp = call_genai_chat(prompt, model=llm_model, max_length=llm_max_tokens, temperature=llm_temp)
...
if st.button("Ask AI"):
    ...
    latest_infra = infra_trend.tail(6).to_dict(orient='records')
    prompt = (
        f"Top counties: {top_snapshot}. Recent national infra trend (last 6): {latest_infra}. "
        f"Question: {qa_input}. Answer concisely with numbers when possible."
    )
    answer = call_genai_chat(prompt, model=llm_model, max_length=llm_max_tokens, temperature=llm_temp)
    st.session_state['last_ai'] = answer
    st.success(answer)
...
if st.button("Generate Executive Summary"):
    top10 = county_demand.sort_values('Monthly_GWh_Demand', ascending=False).head(10)
    prompt = (
        f"Create a 5-7 sentence executive summary for planners given these top 10 counties: "
        f"{top10.to_dict(orient='records')}. Mention actions and grid concerns."
    )
    summary = call_genai_chat(prompt, model=llm_model, max_length=llm_max_tokens, temperature=llm_temp)
    st.session_state['last_ai'] = summary
    st.info(summary)
